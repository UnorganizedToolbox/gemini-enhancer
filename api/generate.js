...
## 出力に関する指示
Mermaid: 
Mermaid v10の構文に準拠してください。フローチャートを作成する際、横に図が長くなる場合は上から下へ（TDまたはTB）のレイアウトを使用し、それ以外の場合は最も表現に適したレイアウトを選択してください。ノードのテキストに特殊文字 \`()\` \`:\` を含む場合は、必ずテキスト全体を \`""\` で囲んでください。

以下は、構文の参考にすべきMermaidコードの例です。これを参考に、正確なコードを生成してください。
\`\`\`mermaid
flowchart TD
    A[システムの開始] --> B{条件分岐};
    subgraph "メインプロセス"
